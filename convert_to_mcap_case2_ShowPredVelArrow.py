...
def write_scene_to_mcap(nusc: NuScenes, nusc_can: NuScenesCanBus, scene, filepath, pred_anns, pred_anns2, case_infos, use_case):
    ...
    with open(filepath, "wb") as fp:
        ...
        while cur_sample is not None:
            ...
            # 网络baseline版本
            scene_update_pred = SceneUpdate()            # cube
            pred_id_scene_update = SceneUpdate()         # 预测目标的id
            pred_velo_x_scene_update = SceneUpdate()     # 预测目标在自车系旋转下vx
            pred_velo_y_scene_update = SceneUpdate()     # 预测目标在自车系旋转下vy
            pred_veloArrow_scene_update = SceneUpdate()  # 预测目标速度箭头
            pred_heading_scene_update = SceneUpdate()    # 预测目标的heading

            scene_update_pred2 = SceneUpdate()         
            pred_id_scene_update2 = SceneUpdate()
            pred_velo_x_scene_update2 = SceneUpdate()
            pred_velo_y_scene_update2 = SceneUpdate()
            pred_veloArrow_scene_update2 = SceneUpdate() 
            pred_heading_scene_update2 = SceneUpdate() 


            anns = pred_anns[cur_sample['token']]
            anns2 = pred_anns2[cur_sample['token']]
...
def convert_all(
    ...
    with open(pred_json2) as J:
        pre_result_infos2 = json.load(J)
        pre_result2 = pre_result_infos2['results']   
    ...
    for scene in nusc.scene:
        ...
        write_scene_to_mcap(nusc, nusc_can, scene, output_dir / mcap_name, pre_result, pre_result2, case_infos, use_case)
