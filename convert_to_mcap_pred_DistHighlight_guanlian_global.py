...
def write_scene_to_mcap(nusc: NuScenes, nusc_can: NuScenesCanBus, scene, filepath, pred_anns, disthighlight_info, guanlian_info, idsw_info):
    ...
    with open(filepath, "wb") as fp:
        ...
        while cur_sample is not None:
            ...
            guan_Line_scene_update = SceneUpdate()
            HighLight_pred = SceneUpdate()
            pred_id_scene_update = SceneUpdate()
            scene_update_pred_idsw = SceneUpdate()
            ...
            pred_highlight_infos = []
            guanlian_idpairs = {}
            pred_idsw_infos = []

            if str(stamp.to_nsec())[:-3] in disthighlight_info.keys():
                pred_highlight_infos = disthighlight_info[str(stamp.to_nsec())[:-3]]   # list  预测结果中因为距离原理gt的高亮目标
            if str(stamp.to_nsec())[:-3] in guanlian_info:
                guanlian_idpairs = guanlian_info[str(stamp.to_nsec())[:-3]]   #用于可视化关联关系 dict
            if str(stamp.to_nsec())[:-3] in idsw_info.keys():
                pred_idsw_infos = idsw_info[str(stamp.to_nsec())[:-3]]   # list
            ...
            for ann in anns:
                ...
                guanpre_anns[ann["tracking_id"]] = ann["translation"]
                ...
                texts_velo_x.pose.position.x = ann["translation"][0]
                texts_velo_x.pose.position.y = ann["translation"][1]
                texts_velo_x.pose.position.z = ann["translation"][2]
                texts_velo_x.pose.orientation.w =  ann["rotation"][0]
                texts_velo_x.pose.orientation.x =  ann["rotation"][1]
                texts_velo_x.pose.orientation.y =  ann["rotation"][2]
                texts_velo_x.pose.orientation.z =  ann["rotation"][3]
                ...
                texts_velo_y.pose.position.x = ann["translation"][0]
                texts_velo_y.pose.position.y = ann["translation"][1]
                texts_velo_y.pose.position.z = ann["translation"][2]
                texts_velo_y.pose.orientation.w = ann["rotation"][0]
                texts_velo_y.pose.orientation.x = ann["rotation"][1]
                texts_velo_y.pose.orientation.y = ann["rotation"][2]
                texts_velo_y.pose.orientation.z = ann["rotation"][3]
            ...
            for ann in anns:
                ...
                if obj_ID in pred_highlight_infos:  # 高亮的预测目标
                    delete_entity_all = HighLight_pred.deletions.add()
                    delete_entity_all.type = 1
                    delete_entity_all.timestamp.FromNanoseconds(stamp.to_nsec() + 50)
                    entity = HighLight_pred.entities.add()
                    entity.frame_id = "map"
                    entity.timestamp.FromNanoseconds(stamp.to_nsec())
                    entity.id = obj_ID
                    entity.frame_locked = True
                    ...
                    cube.color.b = 0
                    cube.color.a = 0.6
            protobuf_writer.write_message("/markers/highlight_pred", HighLight_pred, stamp.to_nsec())
            ...
            for ann in anns:
                ...
                if isinstance(ann["tracking_id"],int):
                    ...
                    obj_ID = ann["tracking_id"]
                if obj_ID in pred_idsw_infos:
                    c = np.array([160, 32, 240]) / 255.0
                    delete_entity_all = scene_update_pred_idsw.deletions.add()
                    delete_entity_all.type = 1
                    delete_entity_all.timestamp.FromNanoseconds(stamp.to_nsec() + 50)
                    entity = scene_update_pred_idsw.entities.add()
                    entity.frame_id = "map"
                    entity.timestamp.FromNanoseconds(stamp.to_nsec())
                    entity.id = obj_ID
                    entity.frame_locked = True
            ...
            protobuf_writer.write_message("/markers/pred_idsw", scene_update_pred_idsw, stamp.to_nsec())        
            ...
            for ann in anns:
                ...
                texts.pose.position.z = ann["translation"][2]
            ...
            for annotation_id in cur_sample["anns"]:
                ...
                guangt_anns[ann["instance_token"]] = ann["translation"]
                cube.pose.position.x = ann["translation"][0]
                cube.pose.position.y = ann["translation"][1]
                cube.pose.position.z = ann["translation"][2]
                cube.pose.orientation.w = ann["rotation"][0]
            ...
            # 关联线
            for gt_id, pred_id in guanlian_idpairs.items():
                if gt_id not in guangt_anns or pred_id not in guanpre_anns:
                    continue
                P1 = guangt_anns[gt_id]
                P2 = guanpre_anns[pred_id] 
                Line_entity = guan_Line_scene_update.entities.add()
                Line_delete_entity_all = guan_Line_scene_update.deletions.add()
                Line_delete_entity_all.type = 1
                Line_delete_entity_all.timestamp.FromNanoseconds(stamp.to_nsec() + 50)

                Line_entity.id = gt_id + pred_id
                Line_entity.frame_id = "base_link"
                ...
                Line.color.b = 0
                Line.color.a = 0.5
                Line.points.add(x = P1[0], y = P1[1], z = P1[2])
                Line.points.add(x = P2[0], y = P2[1], z = P2[2])
            protobuf_writer.write_message("/markers/guanlian_line", guan_Line_scene_update, stamp.to_nsec())
            # pred的idsw  为发生idsw的预测框加上紫色
...
def convert_all(
    ...
    with open(guanlian_json_file,'r') as fp:
        guanlian_infos = json.load(fp)

    with open(DistHighlight_json_file,'r') as fp:
        disthighlight_infos = json.load(fp)

    with open(idsw_json_file,'r') as fp:
        idsw_infos = json.load(fp)
    ...
    for scene in nusc.scene:
        ...
        disthighlight_info = {}
        guanlian_info = {}
        if scene['token'] in disthighlight_infos.keys():
            disthighlight_info = disthighlight_infos[scene['token']]
            guanlian_info = guanlian_infos[scene['token']]

        idsw_info = {}
        if scene_name in idsw_infos.keys():
            idsw_info = idsw_infos[scene_name]

        write_scene_to_mcap(nusc, nusc_can, scene, output_dir / mcap_name, pre_result, disthighlight_info, guanlian_info, idsw_info)
...
def main():
    ...
    parser.add_argument(
        "--idsw_json_name",
        type=str,
        default="idsw_clear_info.json",
    )
    parser.add_argument(
    "--highlight_info",
    type=str,
    default="highlight_info.json",
    )
    ...
    for name in args.dataset_name:
        ...
        if args.list_only:
            nusc.list_scenes()
            return
        pred_json_file = os.path.join(root_injson,args.pred_json_name)
        DistHigh_json_file = os.path.join(root_injson,args.highlight_info)
        guanlian_json_file = os.path.join(root_injson,args.guanlian_json_name)
        idsw_json_file = os.path.join(root_injson,args.idsw_json_name)

        convert_all(args.output_dir, name, nusc, nusc_can, pred_json_file, DistHigh_json_file, guanlian_json_file, idsw_json_file)
